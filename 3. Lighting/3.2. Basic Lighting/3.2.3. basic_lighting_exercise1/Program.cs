...
public class Program {
    ...
    private static void Main(string[] args) {
        ...
        window.RenderFrame += delegate(FrameEventArgs args) {
            ...
            // altera os valores de posição da luz ao longo do tempo (pode ser feito em qualquer lugar do loop de renderização, mas tente fazer isso pelo menos antes de usar as posições da fonte de luz)
            lightPos.X = 1.0f + (float)Math.Sin(GLFW.GetTime()) * 2.0f;
            lightPos.Y = (float)Math.Sin(GLFW.GetTime() / 2.0f) * 1.0f;
