...
public class Program {
    ...
    private static void Main(string[] args) {
        ...
        window.RenderFrame += delegate(FrameEventArgs args) {
            ...
            // ---------------------
            transform = Matrix4.Identity; // redefini-lo para matriz de identidade
            float scaleAmout = (float)Math.Sin(GLFW.GetTime());
            transform *= Matrix4.CreateScale(new Vector3(scaleAmout, scaleAmout, scaleAmout));
            transform *= Matrix4.CreateTranslation(new Vector3(-0.5f, 0.5f, 0.0f));
            GL.UniformMatrix4(transformLoc, false, ref transform); // desta vez pegamos o primeiro elemento do array de valor da matriz como seu valor de ponteiro de memória
