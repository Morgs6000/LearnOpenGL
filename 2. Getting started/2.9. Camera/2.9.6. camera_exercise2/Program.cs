...
public class Program {
    ...
    private static void Main(string[] args) {
        ...
        window.RenderFrame += delegate(FrameEventArgs args) {
            ...
            Matrix4 calculate_lookAt_matrix(Vector3 position, Vector3 target, Vector3 worldUp) {
                // 1. Posição = conhecida
                // 2. Calcule a direção da câmera
                Vector3 zaxis = Vector3.Normalize(position - target);
                // 3. Obtenha vetor positivo do eixo direito
                Vector3 xaxis = Vector3.Normalize(Vector3.Cross(Vector3.Normalize(worldUp), zaxis));
                // 4. Calcule o vetor da câmera para cima
                Vector3 yaxis = Vector3.Cross(zaxis, xaxis);

                // Cria matriz de translação e rotação
                // No glm acessamos elementos como mat[col][row] devido ao layout da coluna principal
                Matrix4 translation = Matrix4.Identity; // Matriz de identidade por padrão
                ...
                // Retorna a matriz lookAt como combinação da matriz de translação e rotação
                return translation * rotation; // Lembre-se de ler da direita para a esquerda (primeiro translação e depois rotação)
            }
            ...
            view *= calculate_lookAt_matrix(new Vector3(camX, 0.0f, camZ), new Vector3(0.0f, 0.0f, 0.0f), new Vector3(0.0f, 1.0f, 0.0f));
