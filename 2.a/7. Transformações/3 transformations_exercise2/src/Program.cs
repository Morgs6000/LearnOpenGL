...
public class Program : GameWindow {
    ...
    protected override void OnRenderFrame(FrameEventArgs args) {
        ...
        Matrix4 trans = Matrix4.Identity; // certifique-se de inicializar a matriz para a matriz identidade primeiro
        trans *= Matrix4.CreateTranslation(new Vector3(1.0f, 1.0f, 0.0f));
        vec = trans * vec;
        ...
        */
        ...
        // cria transformações
        Matrix4 trans = Matrix4.Identity; // certifique-se de inicializar a matriz para a matriz identidade primeiro
        trans *= Matrix4.CreateScale(new Vector3(0.5f, 0.5f, 0.5f));
        trans *= Matrix4.CreateRotationZ(MathHelper.DegreesToRadians(90.0f));
        */

        // cria transformações
        Matrix4 trans = Matrix4.Identity; // certifique-se de inicializar a matriz para a matriz identidade primeiro
        trans *= Matrix4.CreateRotationZ((float)GLFW.GetTime());
        trans *= Matrix4.CreateTranslation(new Vector3(0.5f, -0.5f, 0.5f));

        // obtém a localização uniforme da matriz e define a matriz
        ourShader.use();
        int transformLoc = GL.GetUniformLocation(ourShader.shaderProgram, "transform");
        ...
        trans = Matrix4.Identity; // redefini-lo para matriz de identidade
        float scaleAmout = (float)Math.Sin(GLFW.GetTime());
        trans *= Matrix4.CreateScale(new Vector3(scaleAmout, scaleAmout, 0.0f));
        trans *= Matrix4.CreateTranslation(new Vector3(-0.5f, 0.5f, 0.0f));
        GL.UniformMatrix4(transformLoc, false, ref trans); // desta vez pegamos o primeiro elemento do array de valor da matriz como seu valor de ponteiro de memória

        // agora com a matriz uniforme sendo substituída por novas transformações, desenhe-a novamente.
        GL.DrawElements(PrimitiveType.Triangles, 6, DrawElementsType.UnsignedInt, 0);
